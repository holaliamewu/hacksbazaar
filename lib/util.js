...
export const getTimeAgo = (timestamp) => {
    ...
    if (secondsAgo < 60) {
        return `${secondsAgo <2 ? 'a' : secondsAgo} sec${secondsAgo !== 1 ? 's' : ''} ago`;
    }

    const minutesAgo = Math.floor(secondsAgo / 60);
    if (minutesAgo < 60) {
        return `${minutesAgo <2 ? 'a' : minutesAgo} min${minutesAgo !== 1 ? 's' : ''} ago`;
    }
    ...
    if (monthsAgo < 12) {
        return `${monthsAgo<2 ? 'a' : monthsAgo} month${monthsAgo !== 1 ? 's' : ''} ago`;
    }
